...
const injectNavyTheme = () => {
  ...
  style.textContent = `
    :root{--ink:#000080!important;--muted:#000080!important;--green:#000080!important;--green-2:#000080!important;--rose:#000080!important;--rose-dark:#000080!important;--gold:#000080!important;--sky:#ffffff!important;--blush:#ffffff!important;--cream:#ffffff!important;--paper:#ffffff!important;--card:#ffffff!important;--line:rgba(0,0,128,.16)!important;--shadow:0 24px 70px rgba(0,0,128,.14)!important;--soft-shadow:0 16px 40px rgba(0,0,128,.1)!important}
    body,.centered-header,.site-header,.soft-band,.auth-box,.result-grid div,.receipt-note,.note-band:not(.force-navy){background:#ffffff!important;color:#000080!important}
    .brand-mark,.button.primary,.button:not(.secondary):not(.light),.nav-links a.active,.nav-links a:hover,.scholarship-card.featured,.cta-panel,.note-band,.step span,.google-dot,.chat-row.user .bubble{background:#000080!important;color:#ffffff!important}
    .button.secondary,.button.light,.google-button,.account-actions .button,.limit-pill,input,select,textarea,.surface-card,.feature-card,.university-card,.portal-card,.scholarship-card,.ai-panel,.tip-card,.lookup-card,.result-card,.contact-card,.auth-panel,.auth-card,.admin-panel,.criteria-card,.docs-card,.income-card{background:#ffffff!important;color:#000080!important;border-color:rgba(0,0,128,.16)!important}
    .hero-card,.auth-hero,.contact-hero,.status-hero,.eligibility-hero,.ai-banner{background:linear-gradient(135deg,rgba(0,0,128,.94),rgba(0,0,128,.66)),url("assets/education-hero.png") center/cover!important;color:#ffffff!important}
    ...
    .eyebrow,.surface-card p,.feature-card p,.university-card p,.portal-card p,.scholarship-card p,.scholarship-card li,.tip-card p,.lookup-form small,.result-message,.receipt-note,.contact-card p,.auth-panel p,.criteria-card p,.docs-card p,.income-card p{color:#000080!important}
  `;
...
const injectMobileNavFix = () => {
  ...
  style.textContent = `
    ...
    @media (max-width: 920px) {
      .nav-toggle{display:block!important;flex:0 0 auto!important}.centered-nav{justify-content:space-between!important;gap:12px!important}.centered-nav .logo-only{position:static!important;transform:none!important}.centered-nav .nav-links{position:fixed!important;inset:76px 12px auto 12px!important;display:none!important;flex-direction:column!important;align-items:stretch!important;gap:6px!important;padding:12px!important;border-radius:14px!important;background:rgba(255,255,255,.96)!important;box-shadow:0 24px 70px rgba(0,0,128,.22)!important;z-index:90!important;white-space:normal!important}.centered-nav .nav-links a{width:100%!important;text-align:left!important;padding:13px 14px!important}body.nav-open .centered-nav .nav-links{display:flex!important}
    }
